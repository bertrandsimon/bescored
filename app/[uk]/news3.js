...
import fallbackPosts from "./datas/newsPosts";

export default function News3() {
  const [latest, setLatest] = useState(fallbackPosts.slice(0, 3));

  useEffect(() => {
    fetch("/api/news?locale=en")
      .then((res) => res.json())
      .then((data) => {
        if (data.ok && Array.isArray(data.posts) && data.posts.length) {
          setLatest(data.posts.slice(0, 3));
        }
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 lg:px-8 mb-40">
      ...
      <div className="flex justify-center pt-16">
        <Link href="/uk/news" className="btn btn-blue oswald">
          Learn more
        </Link>
      </div>
    </div>
