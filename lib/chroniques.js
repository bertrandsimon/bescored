...
function upgradeDisplayImageUrl(url = "") {
  if (!url) return "";
  return url.replace(/shrink_(\d+)(?:_(\d+))?/g, (_, width, height) => {
    const min = 1280;
    const nextWidth = Math.max(Number(width) || 0, min);
    if (height) {
      return `shrink_${nextWidth}_${Math.max(Number(height) || 0, min)}`;
    }
    return `shrink_${nextWidth}`;
  });
}
...
export function mapChronique(doc, locale = "fr") {
  ...
  const image = upgradeDisplayImageUrl(
    sanitizeImageUrl(locale === "en" ? doc.image_uk : doc.image_fr)
  );
